...
SELECT
    a.cnpj AS id_escola,
    COALESCE(c.nome_escola, ea.nome_escola, eb.nome_escola, z.nome_escola) AS nome_escola,
    LEAST(c.created_date, ea.created_date, eb.created_date, z.created_date) AS data_cadastro,
    c.cnpj IS NOT NULL AS has_crm,
    ea.cnpj IS NOT NULL AS has_erp_a,
    eb.cnpj IS NOT NULL AS has_erp_b,
    z.cnpj IS NOT NULL AS has_zendesk
FROM all_cnpjs a
